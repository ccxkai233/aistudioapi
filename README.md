...
| `CONCURRENCY_MODE`              | 并发模式："single" (传统串行单账号切换) 或 "pool" (多账号高并发池模式)。池模式能够通过负载均衡同时处理大量请求。                                                      | `pool`    |
| `STICKY_THRESHOLD`              | 粘性会话阈值（仅池模式）。同一个 IP 连续路由到同一个账号处理的请求次数上限。这有助于上下文的连贯性。                                                                  | `10`      |
| `REST_DURATION_MINUTES`         | 休息时长（分钟，仅池模式）。账号遇到非 429 的错误（如熔断机制）时的临时休息时间。                                                                                     | `1`       |
| `POOL_BATCH_SIZE`               | 批次大小（仅池模式）。将所有账号按该大小进行分批处理。当一个批次触发了轮换条件后，才会切换到下一个批次，节约账号连通成本。设为 `0` 以禁用（所有账号都在一个大池中）。 | `0`       |
| `POOL_BATCH_SWITCH_RATIO`       | 批次轮换阈值（仅池模式）。当前活跃批次中由于 429 休息的账号数量达到此比例时（如 0.5 表示 50%），自动将流量切换到下一个批次。                                          | `0.5`     |
| `RETIRE_RECOVERY_HOURS`         | 退休恢复时间（小时，仅池模式）。当账号因为严重的 429 报错被永久退休时，经过改配置设置的小时数之后将会自动尝试重载恢复并排至队尾。                                     | `5`       |
